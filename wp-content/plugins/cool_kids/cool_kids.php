...
echo "here";
    if ( ! empty( $intersect_roles ) ) {
        ...
        <table class="form-table">
            <tr>
                ...
                <td>
                    <input type="text" name="cool_kid_country" id="cool_kid_country" value="<?php echo esc_attr( get_the_author_meta( 'cool_kid_country', $user->ID ) ); ?>" class="regular-text" />
                </td>
            </tr>
        </table>
        <?php
	...
	private function setup_shortcodes() {
		...
		add_shortcode( 'cool_kid_login', array( $this, 'shortcode_login' ) );
	}
...
//return ob_get_clean();		//return ob_get_clean();
	...
	private function enqueue_assets() {
		...
		function(){
			...
			//alpine js
			wp_enqueue_script( 'alpinejs', 'https://cdn.jsdelivr.net/gh/alpinejs/alpine@v2.x/dist/alpine.min.js');
		});
	}
	...
	public function rest_login($data){
		...
		if ( ! $user ) {
			return new WP_Error( 'rest_login_failed', esc_html__( 'Invalid email 2' ). $data['email'], array( 'status' => 403,'error'=>true ) );
		}
		$creds = array();
		...
		if ( is_wp_error( $user ) ) {
			return new WP_Error( 'rest_login_failed', esc_html__( 'Invalid email' ), array( 'status' => 403,'error'=>true ) );
		}
		return array( 'message' => 'User logged in successfully','success'=>true );
	}
	...
	public function verify_nonce( $request ) {
		...
		if ( ! wp_verify_nonce( $nonce, 'wp_rest' ) ) {
			return new WP_Error( 'rest_forbidden', esc_html__( 'Invalid nonce'.$nonce ), array( 'status' => 403,'error'=>true ) );
		}
		return true;
	}
